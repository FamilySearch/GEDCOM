...
- Zip allows varying levels of compression and can compress different files differently. Adding zip compression to an already-compressed file (such as most image and video files) tends to be slow and to not shrink the file. Applications may find performance improved by not applying compression at all or by applying compression only to the text files (including `gedcom.ged`) within the GEDZIP archive.
:::
